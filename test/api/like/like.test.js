...
describe('GET /most-liked', () => {
  ...
  it('should return array of cakes and status 200', done => {
      chai.request(app).get('/most-liked')
        ...
        // .to.have.nested.property("cakes[0]")
        // .that.includes.all.keys([ '_id', 'user_name', 'email', 'likes', 'like_count' ])
  });
});

describe('POST /cake/like', () => {
   // Like User
   it('Should return message and status 200', done => {
    chai.request(app).post('/cake/like').set("Authorization", 'Bearer ' + token)
    .end((error, res) => {
...
describe('POST /cake/unlike', () => {
  // Unlike User
  it('Should return message and status 200', done => {
    chai.request(app).post('/cake/unlike').set("Authorization", 'Bearer ' + token)
    .end((error, res) => {
    expect(res).to.be.json;
    expect(res.body).to.be.an.instanceof(Object)
