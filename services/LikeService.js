...
module.exports = {
  setLike: async function (userId, cakeId) {
    try {
      if(!userId) {
        return {
          status: 403,
        ...
        let like = await pool.query('SELECT * FROM likes WHERE user_id = $1 AND cake_id = $2', [userId, cakeId])
        if(like){
          return {
            status: 200,
          ...
          if(cake){
            const res = await pool.query('INSERT INTO likes (user_id, cake_id) VALUES ($1, $2, $3, $4, $5)',[userId, cakeId])
            return {
              status: 200,
